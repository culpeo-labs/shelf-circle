...
- **development** / **preview** — installable Android APK + iOS _simulator_
  ...
  GitHub Actions run. No build runs automatically on merge: EAS's Free plan has
  a small monthly build quota, so builds are batched by hand (see production,
  below).
- **production** — real Play Store AAB, and (once there's an Apple
  Developer account to sign with) an App Store IPA. Android goes to
  **Play internal testing** on demand: run the Frontend Internal Testing
  workflow (Actions tab, or `gh workflow run frontend-internal-testing.yml`)
  when enough has landed on `main` to be worth a build. It builds this profile
  and submits it to the `internal` track (`eas.json` `submit.internal`,
  `releaseStatus: completed`). It's manual, not per-merge, so changes batch
  into few builds. To go to production, **promote the baked release in Play
  Console** (internal testing → production) rather than rebuilding; the manual
  `frontend-store-submit.yml` is the alternative that builds fresh and
